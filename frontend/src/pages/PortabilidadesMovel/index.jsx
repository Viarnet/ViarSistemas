...
import './styles.css'
export default function PortabilidadesMovel() {
   const [portabilidades, setPortabilidades] = React.useState([]);
   const [loading , setLoading] = React.useState(false)
   useEffect(()=>{
    setLoading(true)
    axios.get(`http://192.168.0.95:3333/telefonia-fixa/portabilidades-movel`).then(({data})=>{
  ...
  return (
      ...
      <h2 style={{marginBottom: 25}}>Portabilidades de Movel</h2>
        {loading && <MiniLoading />}
